	...
	$("#btnDialogApply").click(function(){
		...
		Req.SetCallBackFunc(function(xml){
			...
			if(ret != 0) {
				...
				var tmp_getlang = parent.g_configData.lang;
				var tmp_findstr = errormessage.indexOf("root");
				
				if(tmp_findstr == -1)
					alert(GetMsgLang("0501") + errormessage);
				else
					if(tmp_getlang == "/language/Korean.xml")
						alert("\’root\’는 예약되어 있어서 사용할 수 없습니다.");
					else if(tmp_getlang == "/language/Chinese.xml")
						alert("\'root\' 被保留，所以請使用其他用戶名。");
					else
						alert("\'root\' is reserved, so please use other username.");
			}
